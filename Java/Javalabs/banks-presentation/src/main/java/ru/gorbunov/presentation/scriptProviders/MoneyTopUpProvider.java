...
import ru.gorbunov.presentation.scripts.TopUpMoney;
...
@Component
@Scope("prototype")
public class MoneyTopUpProvider implements ScriptProvider {
    private final CurrentUser currentUser;
    private final BankService service;

    public MoneyTopUpProvider(CurrentUser currentUser, BankService service) {
    ...
    public Optional<Script> tryGetScenario() {
        if (currentUser.user.isPresent() && currentUser.user.get().getRole() == Roles.CLIENT)
            return Optional.of(new TopUpMoney(service));
