...
import ru.gorbunov.presentation.scripts.Transfer;
...
@Component
@Scope("prototype")
public class TransferProvider implements ScriptProvider {
    private final CurrentUser currentUser;
    private final BankService service;

    public TransferProvider(CurrentUser currentUser, BankService service) {
    ...
    public Optional<Script> tryGetScenario() {
        if (currentUser.user.isPresent() && currentUser.user.get().getRole() == Roles.CLIENT) {
            return Optional.of(new Transfer(service));
        }
